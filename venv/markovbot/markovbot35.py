        ...
        for targetstring in respdict.keys():
            ...
            if type(targetstring) not in [str]:
                self._message(u'set_simple_responses', \
                              u"Key '%s' in passed respdict is not a string or unicode type, and thus will not be included in the database." % (
                                  targetstring))
                continue
            ...
            # For any other data type, report a warning.
                ...
                self._message(u'set_simple_responses', \
                              u"Key '%s' in passed respdict points to invalid data. Values need to be of type str, unicode, list, or tuple." % (
                                  targetstring))
    ...
    def twitter_autoreply_start(self, targetstring, database=u'default',
        ...
        if database == u'simpleresponse':
            if targetstring not in self.data[u'simpleresponse'].keys():
                self._error(u'twitter_autoreply_start', \
                            u"Targetstring '%s' was not found in the 'simpleresponse' database. Use the set_simple_responses function to add simple responses." % (
                                targetstring))
